...
Position getAGoodMove(char board[BOARD_WIDTH][BOARD_WIDTH]) {
	...
	//cout << searchResult.x << "  " << searchResult.y <<endl;
...
void firstPoint(Position p)
{
	int x = p.x;
	int y = p.y;
    currentZobristValue ^= boardZobristValue[COMPUTOR - 1][x][y];
    board[x][y] = COMPUTOR;
    updateScore(board, Position(x, y));

    moves.push(Position(x, y));
    searchResult = Position(x, y);
